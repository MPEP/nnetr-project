...
Für den Trainingsprozess wird der Rückgabewert der Aktivierungsfunktion iterativ für alle Eingabeinstanzen ermittelt und mit der tatsächlichen Wertausprägung der Zielvariablen $y_i$ eines Trainingsdatensatzes $\vec{y}_{opt}$ abgeglichen. Falls das Ausgangssignal $z$ nicht mit dem Wert des Trainingsdatensatzes übereinstimmt, wird der Gewichtungsvektor $\vec{c}$ der $c$-ten Iteration durch den angepassten Vektor $\vec{w_{c+1}}$ ersetzt. Der gesamte Algorithmus ist wie folgt definiert:
