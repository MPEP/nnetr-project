...
\begin{algorithmic}[1]
\Procedure{Perzeptron}{$x$}\Comment{$x$ entspricht}
\State $w_0\gets 0$
\State $b_0\gets 0$
\State $c\gets 0$
\State $\text{R}\gets \text{max}_{1 \leq i \leq l} \, \lVert \mathbf{x_i} \rVert$
\State $\text{weightUpdate}\gets \text{TRUE}$
\While{$\text{weightUpdate}$}
\State $\text{weightUpdate}\gets \text{FALSE}$
\State $y_\text{classified}\gets \text{Sign}(\langle \mathbf{w_i} \cdot x_i \rangle + b_c)$ 
\For{$i = 1$ to $l$}
\If{$y_i \leq 0$}
\State $w_{c+1}\gets w_c + \eta y_i x_i$
\State $b_{c+1}\gets b_c + \eta y_i \text{R}^2$
\State $c\gets c+1$
\EndIf
\EndFor
\EndWhile
