...
Für das Perzeptron-Modell wurden, nach dem Vorbild der Funktionen zur Erstellung von Regressionsmodellen, die gängigsten generischen Funktionen implementiert. Hierzu zählen die Funktionen \texttt{print.perceptron()}, \texttt{summary.perceptron()} und \texttt{plot.perceptron()} zu denen nähere Informationen den jeweiligen Hilfseiten entnommen werden können.
...
```{r, fig.align='center', out.width = "70%", fig.cap="Plot der Kelch- gegen die Kronenblattlänge in cm. Clusterschema deutet auf lineare Sparierbarkeit der Stichprobe hin."}
